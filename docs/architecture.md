...
- **참고안은 만들지 않고 흘린다.** 복합 완료 답변의 임상 참고안은 BE 완결이 구조화·검증·조립해 답변과 같은 tx에 세우고, 에이전트는 완결 응답의 `guidance`를 `message`에서 떼어 `answer.completed{message, guidance}`로 보낸다 — 참고안은 판단이 아니라 저장물이므로 프롬프트·검증 규칙·킬스위치가 BE에 한 벌로 남는다(BE `docs/specs/54`). 참고안을 만든 완결에만 그 키가 있어, 없으면 이벤트에도 키를 만들지 않는다.
- **완결만 읽기를 오래 기다린다.** 완결 호출은 connect 5초 · read 30초이고(`backend.BACKEND_TIMEOUT`이 아니라 `FINISH_TIMEOUT`) 그 밖의 JSON 호출은 5초 그대로다 — BE가 완결 안에서 참고안을 구조화하기 때문이다(BE 상한 20초). connect를 넓히지 않는 이유는 늘린 대상이 「BE가 일하는 동안의 기다림」이지 BE 순단이 아니기 때문이다. 실행 상한 150초는 그대로 둔다 — 구조화 실측이 3초 미만이라 상한을 올려 선검사 401을 늘릴 이유가 없다.
