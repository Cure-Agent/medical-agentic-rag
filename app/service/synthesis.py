...
# (BE §42와 같은 이유)
PATIENT_PROMPT_VERSION = "agent-patient-v2"
COMPOSITE_PROMPT_VERSION = "agent-composite-v2"
...
def patient_messages(
    ...
    rules = "\n".join(
        [
            ...
            "   진단·처방 판단을 새로 만들지 않는다. 「이 약을 써도 되는가」처럼",
            "   가부를 묻는 질문에도 판단을 내리지 않는다 — 관련된 기록 항목",
            "   (알레르기·병력·투약)을 그대로 전하고, 판단은 의료인의 몫이라고 밝힌다.",
...
def composite_messages(
    ...
    rules = "\n".join(
        [
            ...
            "   기간·금기를 일반 지식으로 보충하지 않는다. 치료 시작 시점·시기 같은",
            "   판단도 근거에 없으면 말하지 않는다.",
            "2. 근거를 사용한 문장에는 그 근거의 마커를 [n] 형식으로 표기한다",
            "   (예: 침 치료가 권고된다 [1]). 질문과 무관한 근거에는 마커를 달지",
            "   않는다 — 마커를 단 근거는 모두 인용으로 저장된다. 근거의 조건을",
            "   바꾸지 않는다 — 「A와 B의 병행」 권고를 「A 단독」 권고로, 비교",
            "   권고를 단독 허용으로 옮기지 않는다.",
            "3. 환자 기록의 진단·투약·알레르기·임상 메모가 근거의 조건·금기와",
            "   맞닿으면 그 점을 짚는다.",
            "4. 기록과 근거로 이 환자에 대한 질문에 답할 수 없으면",
            "   insufficientEvidence를 true로 두고 answer는 비운다. 근거가 질문의",
            "   질환·환자군과 다르면(다른 질환의 지침, 성인 근거에 소아 환자 등)",
            "   답할 수 없는 것이다. 답할 수 있으면 false로 두고 answer에 답을 쓴다.",
            "   answer에 「적용하기 어렵다」류의 거부 산문을 쓰지 않는다.",
