...
> **결과 요약 (2026-08-24)**: 가설은 틀렸다. 병목이 순위 문제였던 것은 맞지만, 그것을 푼 것은
> 분해가 아니라 **리랭커**였다(complex 0.17 → 0.50). **분해·근거 증량·축 열거 프롬프트·반복 검색
> 루프 — 에이전틱 개입 네 개가 모두 기각됐고, 검색 경로는 운영 현행(원 질문 1개)을 유지한다.**
> 대신 예상 밖의 소득이 있었다 — 「관련도」와 「답변가능성」이 다른 질문이라는 것, 그리고
> 1회 실행 비교가 얼마나 쉽게 뒤집히는지. 자세한 내용은 [실험 기록](docs/experiments/INDEX.md).
...
**현재 판정: 검색 경로는 운영 현행을 유지한다.** 제안됐던 네 개입이 모두 기각됐다
